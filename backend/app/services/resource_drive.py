...
from app.repositories.resources import DriveCredentialRepository, ResourceRepository
...
class ResourceDriveService:
    ...
    async def build_connect_url(self, user_id: str) -> Optional[str]:
    ...
    async def handle_oauth_callback(self, state: str, code: str) -> bool:
        user_id = await self._oauth_state.consume(state, "drive")
        if not user_id:
            return False
        success = await self.handle_callback(user_id, code)
        if not success:
            return False
        try:
    ...
    async def refresh(self, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def sync(self, user_id: str) -> Dict[str, Any]:
        ...
        subj_by_name = {subject["name"].lower(): subject["subject_id"] for subject in subjects}
        ...
        for type_folder in type_folders:
            ...
            subject_folders = GoogleDriveIntegration.list_children(
                service, type_folder["id"], mime_type=folder_mime
            )
            for subject_folder in subject_folders:
                subject_name = subject_folder["name"]
                subject_id = subj_by_name.get(subject_name.lower())
                if not subject_id:
                    if subject_name not in unknown_subjects:
                        unknown_subjects.append(subject_name)
                    continue
                batch: List[Dict[str, Any]] = []
                files = GoogleDriveIntegration.list_children(
                    service, subject_folder["id"]
                )
                for drive_file in files:
                    if drive_file["id"] in existing_ids:
                        skipped += 1
                        continue
                    batch.append({
                        "resource_id": new_id("res"),
                        ...
                        "external_url": drive_file.get("webViewLink", ""),
                        "source": "drive",
                        "created_at": drive_file.get("createdTime") or iso(now_utc()),
                        "synced_at": iso(now_utc()),
                    })
                    existing_ids.add(drive_file["id"])
                if batch:
    ...
    async def view_resource(
        ...
        if not drive_file_id and res.get("external_url"):
            match = re.search(r"/file/d/([A-Za-z0-9_-]+)", res["external_url"])
            if not match:
                match = re.search(r"[?&]id=([A-Za-z0-9_-]+)", res["external_url"])
            if match:
    ...
    async def stream_resource(
        ...
        try:
            content = await download(creds.token)
