...
import os, re, uuid, logging, requests
...
async def list_questions(
    ...
    if question_type: q["question_type"] = question_type
...
async def import_playlist(body: PlaylistImportIn, user=Depends(get_current_user)):
    ...
    try:
        ...
        snip = meta["items"][0]["snippet"]
        item_count = meta["items"][0]["contentDetails"]["itemCount"]
        # fetch items
        videos: List[Dict[str, Any]] = []
        token = None
        while True:
            params = {"part": "snippet,contentDetails", "playlistId": pid,
                      "maxResults": 50, "key": YOUTUBE_API_KEY}
            if token: params["pageToken"] = token
            ri = requests.get("https://www.googleapis.com/youtube/v3/playlistItems",
        ...
        for i in range(0, len(ids), 50):
            ...
            rv = requests.get("https://www.googleapis.com/youtube/v3/videos",
                              params={"part": "contentDetails", "id": ",".join(chunk),
                                      "key": YOUTUBE_API_KEY}, timeout=15).json()
            for it in rv.get("items", []):
                durations[it["id"]] = _iso8601_to_seconds(it["contentDetails"]["duration"])
    except Exception as e:
    ...
    playlist = {
        ...
        "subject_id": body.subject_id, "youtube_playlist_id": pid,
        "title": snip["title"],
        "thumbnail": (snip.get("thumbnails", {}).get("high") or snip.get("thumbnails", {}).get("default", {})).get("url", ""),
        "channel_title": snip.get("channelTitle", ""),
        "video_count": item_count, "created_at": iso(now_utc()),
    }
...
async def dashboard(user=Depends(get_current_user)):
    ...
    distinct_q = len({a["question_id"] for a in qa})
    distinct_p = len({a["pyq_id"] for a in pa})
    q_correct = sum(1 for a in qa if a["is_correct"])
    p_correct = sum(1 for a in pa if a["is_correct"])
    q_acc = round(q_correct / len(qa) * 100, 1) if qa else 0
    p_acc = round(p_correct / len(pa) * 100, 1) if pa else 0
    ...
    for s in subjects:
        ...
        # solved by user
        all_qs = await db.questions.find({"subject_id": sid}, {"_id": 0, "question_id": 1}).to_list(5000)
        qids = [q["question_id"] for q in all_qs]
        ua = [a for a in qa if a["question_id"] in qids]
        qb_solved = len({a["question_id"] for a in ua})
        qb_correct = sum(1 for a in ua if a["is_correct"])
        qb_acc = round(qb_correct / len(ua) * 100, 1) if ua else 0
        all_ps = await db.pyqs.find({"subject_id": sid}, {"_id": 0, "pyq_id": 1}).to_list(5000)
        pids = [p["pyq_id"] for p in all_ps]
        upa = [a for a in pa if a["pyq_id"] in pids]
        pyq_solved = len({a["pyq_id"] for a in upa})
        pyq_correct = sum(1 for a in upa if a["is_correct"])
        pyq_acc = round(pyq_correct / len(upa) * 100, 1) if upa else 0
        overview.append({
            "subject": s,
            "qb": {"total": qb_total, "solved": qb_solved, "remaining": qb_total - qb_solved, "accuracy": qb_acc},
            "pyq": {"total": pyq_total, "solved": pyq_solved, "remaining": pyq_total - pyq_solved, "accuracy": pyq_acc},
        })
    recent = sorted(
        [{"type": "question", **a} for a in qa[-5:]] + [{"type": "pyq", **a} for a in pa[-5:]],
        key=lambda x: x["attempted_at"], reverse=True,
    )[:10]
    return ok({
        "summary": {
            "questions_solved": distinct_q, "pyqs_solved": distinct_p,
            "videos_completed": videos_done, "total_playlists": playlists,
            "question_accuracy": q_acc, "pyq_accuracy": p_acc,
            "total_mistakes": mistakes, "resources_uploaded": resources,
        },
        "subjects": overview, "recent_activity": recent,
    })

@api.get("/analytics/subject/{subject_id}")
async def subject_analytics(subject_id: str, user=Depends(get_current_user)):
    uid = user["user_id"]
    ...
    for t in topics:
        tid = t["topic_id"]
        qs = await db.questions.find({"topic_id": tid}, {"_id": 0, "question_id": 1}).to_list(5000)
        qids = [q["question_id"] for q in qs]
        ua = [a for a in qa if a["question_id"] in qids]
        ps = await db.pyqs.find({"topic_id": tid}, {"_id": 0, "pyq_id": 1}).to_list(5000)
        pids = [p["pyq_id"] for p in ps]
        upa = [a for a in pa if a["pyq_id"] in pids]
        ...
        out.append({
            "topic": t,
            "qb": {"total": len(qs), "solved": len({a["question_id"] for a in ua}),
                   "remaining": len(qs) - len({a["question_id"] for a in ua}),
                   "accuracy": round(sum(1 for a in ua if a["is_correct"]) / len(ua) * 100, 1) if ua else 0},
            "pyq": {"total": len(ps), "solved": len({a["pyq_id"] for a in upa}),
                    "remaining": len(ps) - len({a["pyq_id"] for a in upa}),
                    "accuracy": round(sum(1 for a in upa if a["is_correct"]) / len(upa) * 100, 1) if upa else 0},
            "notes_count": notes, "mistakes_count": mis,
        })
    return ok(out)

@api.get("/analytics/topic/{topic_id}")
async def topic_analytics(topic_id: str, user=Depends(get_current_user)):
    uid = user["user_id"]
    t = await db.topics.find_one({"topic_id": topic_id}, {"_id": 0})
    if not t:
        return err("not_found", "Topic not found", 404)
    qs = await db.questions.find({"topic_id": topic_id}, {"_id": 0, "question_id": 1}).to_list(5000)
    qids = [q["question_id"] for q in qs]
    ps = await db.pyqs.find({"topic_id": topic_id}, {"_id": 0, "pyq_id": 1}).to_list(5000)
    pids = [p["pyq_id"] for p in ps]
    qa = await db.question_attempts.find({"user_id": uid, "question_id": {"$in": qids}}, {"_id": 0}).to_list(20000)
    pa = await db.pyq_attempts.find({"user_id": uid, "pyq_id": {"$in": pids}}, {"_id": 0}).to_list(20000)
    notes = await db.question_notes.count_documents({"user_id": uid, "question_id": {"$in": qids}})
    mis = await db.mistakes.count_documents({"user_id": uid, "topic_id": topic_id})
    return ok({
        "topic": t,
        "qb": {"total": len(qs), "solved": len({a["question_id"] for a in qa}),
               "remaining": len(qs) - len({a["question_id"] for a in qa}),
               "accuracy": round(sum(1 for a in qa if a["is_correct"]) / len(qa) * 100, 1) if qa else 0},
        "pyq": {"total": len(ps), "solved": len({a["pyq_id"] for a in pa}),
                "remaining": len(ps) - len({a["pyq_id"] for a in pa}),
                "accuracy": round(sum(1 for a in pa if a["is_correct"]) / len(pa) * 100, 1) if pa else 0},
        "notes_count": notes, "mistakes_count": mis,
    })
        ...
        for i, (name, topics) in enumerate(SUBJECTS_SEED):
            ...
            for j, tname in enumerate(topics):
                ...
                await db.topics.insert_one({
                    "topic_id": tid, "subject_id": sid, "name": tname,
                    "order": j, "created_at": iso(now_utc()),
                })
    # build name → ids map
    subs = {s["name"]: s async for s in db.subjects.find({}, {"_id": 0})}
    tops = {(t["subject_id"], t["name"]): t async for t in db.topics.find({}, {"_id": 0})}
